...
export const CanvasRevealEffect = ({
  ...
  colors = [
    [240, 240, 240], // soft white gray
    [220, 220, 220], // warm silver
    [200, 200, 200], // cool stone
    [180, 170, 160], // taupe
    [160, 150, 140], // warm gray
    [190, 180, 170], // light mocha
  ],
  containerClassName,
  dotSize = 3,
  showGradient = true,
...
const DotMatrix: FC<DotMatrixProps> = ({
  colors = [[0, 0, 0]],
  opacities = [0.04, 0.04, 0.08, 0.08, 0.14],
  totalSize = 4,
  dotSize = 2,
  shader = '',
  center = ['x', 'y'],
  ...
  return {
    ...
    u_opacities: {
      value: opacities.map((v) => Math.min(1, Math.max(0.3, v))),
      type: "uniform1fv",
    },
    u_total_size: { value: totalSize * 1.1, type: "uniform1f" },
    u_dot_size: { value: dotSize * 1.2, type: "uniform1f" },
  };
...
const ShaderMaterial: FC<ShaderProps> = ({ source, uniforms, maxFps = 60 }) => {
  const { size } = useThree();
